...
#'  plot_covar_trace(x=out, j=1, n0=n0, ntotal=ntotal)

plot_covar_trace <- function(x, j, n0, ntotal, xlab = "iter", ylab = paste("encounter age effect at j=",j), ...){
  iter <- (n0 + 1):ntotal
  trace <- x$trace3[iter, j]
